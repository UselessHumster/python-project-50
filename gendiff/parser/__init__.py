...
def has_difference(child1, child2):
    if not child1:
        return False
    if not child2:
        return False
...
def check_flat_diff(child1, child2, acc):
    if has_difference(child1, child2):
        acc.append(child1)
    if has_been_removed(child1, child2):
        name = get_name(child1)
        value = get_value(child1)
        acc.append(mk_flat_diff(name, value, 'removed'))
    if has_been_added(child1, child2):
        name = get_name(child2)
        value = get_value(child2)
        acc.append(mk_flat_diff(name, value, 'added'))
    return acc
